...
def p_to_k3(sg : float, p : np.ndarray):
    """
...
def kappa_func_k3(s : np.ndarray, sg : float, p : np.ndarray):
   a = p_to_k3(sg, p)
   return ((a[3] * s + a[2]) * s + a[1]) * s + a[0]

def dkappa_func_k3(s : np.ndarray, sg : float, p : np.ndarray):
   a = p_to_k3(sg, p)
   return (3 * a[3] * s + 2 * a[2]) * s + a[1]

def theta_func_k3(s : np.ndarray, sg : float, p : np.ndarray): 
  a = p_to_k3(sg, p)
  return (((a[3] * s / 4 + a[2] / 3) * s + a[1] / 2) * s + a[0]) * s

def partial_theta_p1_k3(s : np.ndarray, sg : float):
  sog = s/sg
  return ((sog * 3.375 - 7.5) * sog + 4.5) * sog * s

def partial_theta_p2_k3(s : np.ndarray, sg : float):
    sog = s/sg
    return ((6.0 - 3.375 * sog) * sog - 2.25) * sog * s

def partial_theta_sg_k3(s : np.ndarray, sg : float, p : np.ndarray):
  sog = s / sg
